...
> POST
	...
	HEADER:
	HTTP_TYPE: getUserRTData
	
	BODY:
	{"user":"cqy"}
	
	RESPONSE:
	{"array": [{"snaddr": "G2100201", "door": "F", "temp": {"status": "0", "value": "18.85"}, "area": "\xe5\x8c\xba\xe5\x9f\x9f1", "pow": "0", "devName": "4G\xe6\xb5\x8b\xe8\xaf\x95\xe6\x9c\xba", "abnormal": "0", "in1": "0FFF", "water": "F", "humi": {"status": "0", "value": "56.64"}, "smoke": "F", "time": "21-03-12 10:18", "press": {"status": "0", "value": "0.00"}}], "code": 0}
	
注解： humi,in1,temp,time为设备最后一次上传数据,
press:压力值，目前不展示
in1：0FFF其实依次代表了 pow smoke water door,"F"：不存在 , "0" ：正常 ,"1"：报警。
数据是否超过了报警阈值，增加一个标志位abnormal，设备温湿度是否超出阈值，增加一个标志位status，-1为低于阈值下限，0为正常处于阈值内，1超出最高阈值，温度和湿度各有一个阈值。
abnormal具体含义定义:
0 -- 无异常;
1 -- 备离线（优先级最高）
2 -- 传感器异常（优先级第二高，一旦传感器异常，无视下列所有异常，损坏）
